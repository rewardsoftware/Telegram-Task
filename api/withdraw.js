...
const USDT_PER_T  = 0.005;  // 1 T Coin = $0.005 USDT (200 T Coin = $1 USDT)
const MIN_T_COIN  = 20;     // min 20 T Coin = $0.10 USDT
const CURRENCY    = 'USDT';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({
      ...
      rate: '20 T Coin = $0.10 USDT (1 T Coin = $0.005 USDT)',
  ...
  try {
    ...
    const params = new URLSearchParams({
      ...
      to:       email,
      amount:   usdtAmount.toFixed(8), // USDT supports decimals
      currency: CURRENCY,
      referral: 'true'
    });
