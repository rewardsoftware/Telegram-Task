...
const T_PER_USD  = 2000;
const MIN_T_COIN = 20;
const CURRENCY   = 'USDT'; // FaucetPay correct currency string
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (usdAmount < 0.01) {
      return res.status(400).json({ ok: false, error: 'Amount too small for FaucetPay (min $0.01)' });
    }
    ...
    if (fpData.status === 200) {
      ...
      // Return full FaucetPay error so we can debug on frontend
      return res.status(200).json({
        ok:     false,
