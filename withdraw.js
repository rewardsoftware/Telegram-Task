...
// FaucetPay API key only from environment variable — never hardcoded
...
export default async function handler(req, res) {
  ...
  if (!FAUCETPAY_API_KEY) {
    return res.status(500).json({ ok: false, error: 'FAUCETPAY_API_KEY not configured in Vercel environment variables' });
  }
